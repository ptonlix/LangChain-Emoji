...
class TencentVectorDB(VectorStore):
    ...
    def __init__(
        ...
        self.ebd_own = (
            True if embedding is None else False
        )  # 是否采用腾讯云自带embedding
    ...
    def _create_collection(self, collection_name: str) -> None:
        ...
        coll = guard_import("tcvectordb.model.collection")
        index_type = None
        ...
        if self.ebd_own:
            ebd = coll.Embedding(
                ...
                field=self.field_text,
                model=enum.EmbeddingModel.BGE_BASE_ZH,
            )
    ...
    def from_texts(
        ...
        **kwargs: Any,
        ...
        if embedding:
            ...
            if index_params is None:
                index_params = IndexParams(dimension=768)
            vector_db = cls(
                embedding=embedding,
                connection_params=connection_params,
                index_params=index_params,
            ...
            vector_db.add_original_texts(texts=texts, metadatas=metadatas, **kwargs)
        return vector_db
        ...
        **kwargs: Any,
        ...
        for start in range(0, total_count, batch_size):
            ...
            for id in range(start, end, 1):
                ...
                if metadatas is not None:
                    metadata = json.dumps(metadatas[id])
                vdb_id = "{}-{}-{}".format(time.time_ns(), hash(texts[id]), id)
                doc = self.document.Document(
                    id=vdb_id,
                    text=texts[id],
                    metadata=metadata,
    ...
    def similarity_search_with_score(
        ...
        **kwargs: Any,
        ...
        if self.ebd_own:
            res = self.similarity_search_with_score_by_text(
                text=[query],
                k=k,
                param=param,
                expr=expr,
    ...
    def similarity_search_with_score_by_text(
        self,
        text: List[str],
        k: int = 4,
        param: Optional[dict] = None,
        ...
        **kwargs: Any,
        ...
        res_data = self.collection.searchByText(
            embeddingItems=text,
            filter=filter,
        ...
        if "documents" not in res_data:
            raise ValueError(res_data)
        res: List[List[Dict]] = res_data.get("documents")
        # Organize results.
    ...
    def similarity_search_with_score_by_vector(
        ...
        **kwargs: Any,
        ...
        filter = None if expr is None else self.document.Filter(expr)
        ef = 10 if param is None else param.get("ef", 10)
        res_data = self.collection.search(
            vectors=[embedding],
            filter=filter,
            params=self.document.HNSWSearchParams(ef=ef),
            retrieve_vector=False,
        ...
        if "documents" not in res_data:
            raise ValueError(res_data)
        res: List[List[Dict]] = res_data.get("documents")  # Organize results.
        ret: List[Tuple[Document, float]] = []
        if res is None or len(res) == 0:
            return ret
        for result in res[0]:
    ...
    def delete_texts_by_ids(
        ...
        for start in range(0, total_count, batch_size):
            ...
            docs = document_ids[start:end]
            res_data = self.collection.delete(
                document_ids=docs, filter=filter, timeout=timeout
            )

            if res_data.get("code") != 0:
                raise ValueError(f"delete texts failed: {res_data}")
            ret.append(res_data)
        return ret


class EmojiTencentVectorDB(TencentVectorDB):
    field_filename: str = "filename"

    def __init__(
        self,
        connection_params: ConnectionParams,
        index_params: IndexParams = IndexParams(768, replicas=0),  # replicas 默认单实例
        database_name: str = "DeepReadDatabase",
        collection_name: str = "EmojiCollection",
        embedding: Optional[Embeddings] = None,
        drop_old: Optional[bool] = False,
    ):
        super().__init__(
    ...
    def _create_collection(self, collection_name: str) -> None:
        ...
        index = vdb_index.Index(
            ...
            vdb_index.FilterIndex(
                self.field_filename, enum.FieldType.String, enum.IndexType.FILTER
            ),
        )
        # Embedding config
        ebd = None
        ...
        self.collection = self.database.create_collection(
            ...
            shard=self.index_params.shard,
            replicas=self.index_params.replicas,
            description="Collection for DeepRead",
            index=index,
            embedding=ebd,
        )

    @classmethod
    def from_texts_with_filename(
        cls,
        filename: str,
        texts: List[str],
        metadatas: Optional[List[dict]] = None,
        connection_params: Optional[ConnectionParams] = None,
        database_name: str = "DeepReadDatabase",
        collection_name: str = "DeepReadCollection",
    ) -> TencentVectorDB:
    ...
    def add_original_texts_with_filename(
        ...
        **kwargs: Any,
        ...
        return self.add_original_texts(
            texts=texts,
            metadatas=metadatas,
            timeout=timeout,
    ...
    def similarity_search_by_filenames(
        ...
        if len(filenames) > 0:
            for i in range(len(filenames) - 1):
                expr += f"{self.field_filename}={filenames[i]} or"
            expr += f"{self.field_filename}={filenames[-1]}"

        return self.similarity_search(query, k=k, expr=expr)
    ...
    def delete_texts_with_filenames(
        ...
        return self.delete_texts_by_ids(
